...
import { PlusIcon, FolderIcon, EditIcon, InstagramIcon, LinkedInIcon, PinterestIcon, MenuIcon, XIcon } from './components/Icons';
      ...
      <nav className="fixed top-0 left-0 right-0 bg-stone-900/90 backdrop-blur-md z-30 border-b border-stone-800">
        <div className="max-w-[1920px] mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            {/* Language Selector */}
            <div className="hidden md:flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-stone-500">
              <button 
        ...
        {/* Mobile Menu Overlay */}
        {isMobileMenuOpen && (
          <div className="fixed inset-0 top-20 bg-stone-900/95 backdrop-blur-xl z-20 md:hidden flex flex-col p-8 animate-[fadeIn_0.2s_ease-out] overflow-y-auto">
            <div className="flex flex-col gap-8 text-2xl font-serif text-stone-400">
              <button 
                  onClick={() => { handleNavClick('projects'); setIsMobileMenuOpen(false); }}
                  className={`transition-colors hover:text-stone-100 text-left ${view === 'projects' ? 'text-stone-100' : ''}`}
              >
                  {t.nav.projects}
              </button>
              <button 
                  onClick={() => { handleNavClick('energie'); setIsMobileMenuOpen(false); }}
                  className={`transition-colors hover:text-stone-100 text-left ${view === 'energie' ? 'text-stone-100' : ''}`}
              >
                  {t.nav.energy}
              </button>
              <button 
                  onClick={() => { handleNavClick('entwicklung'); setIsMobileMenuOpen(false); }}
                  className={`transition-colors hover:text-stone-100 text-left ${view === 'entwicklung' ? 'text-stone-100' : ''}`}
              >
                  {t.nav.development}
              </button>
              <button 
                  onClick={() => { handleNavClick('studio'); setIsMobileMenuOpen(false); }}
                  className={`transition-colors hover:text-stone-100 text-left ${view === 'studio' ? 'text-stone-100' : ''}`}
              >
                  {t.nav.studio}
              </button>
              <button 
                  onClick={() => { handleNavClick('contact'); setIsMobileMenuOpen(false); }}
                  className={`transition-colors hover:text-stone-100 text-left ${view === 'contact' ? 'text-stone-100' : ''}`}
              >
                  {t.nav.contact}
              </button>
            </div>
            ...
            <div className="flex items-center gap-6 mt-12 pt-12 border-t border-stone-800 text-sm font-bold uppercase tracking-widest text-stone-500">
              <button 
